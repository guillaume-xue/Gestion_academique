...
struct CoursView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                ForEach(cours.listeDeCours) { item in
                    CoursRow(item: item, cours: self.cours, etudiant: self.etudiant, enseignants: self.enseignant)
                }
    ...
    func deleteCours(at offsets: IndexSet) {
        for index in offsets {
            let coursToDelete = cours.listeDeCours[index]
            modelContext.delete(coursToDelete)
        }
